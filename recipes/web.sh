...
_vite_css_entry()  { find "$PROJECT_DIR" -maxdepth 2 \( -name index.css -o -name style.css -o -name styles.css -o -name main.css \) 2>/dev/null | sed -n '1p'; }
_vite_main_entry() { find "$PROJECT_DIR" -maxdepth 2 \( -name 'main.tsx' -o -name 'main.jsx' -o -name 'main.ts' -o -name 'main.js' \) 2>/dev/null | sed -n '1p'; }
...
# Wire Tailwind v4 into a Vite React project. The react template already ships a
# vite.config with @vitejs/plugin-react, so we rewrite it to carry both plugins
# (deterministic, no fragile in-place editing) and prepend the import to src/index.css.
_web_react_tailwind_wire() {
    _ext=js; [ "$LANG" = ts ] && _ext=ts
    if [ "$DRY_RUN" = 1 ]; then
        dim "  would write vite.config.$_ext with @vitejs/plugin-react + @tailwindcss/vite and add '@import \"tailwindcss\";' to src/index.css"
        return 0
    fi
...
EOF
    ok "wrote $(basename "$_cfg") with @vitejs/plugin-react + @tailwindcss/vite"
    _css="$PROJECT_DIR/src/index.css"; [ -f "$_css" ] || _css="$(_vite_css_entry)"
    if [ -z "$_css" ] || [ ! -f "$_css" ]; then
        warn "no css entry found — add '@import \"tailwindcss\";' to src/index.css manually"; return 0
    fi
    if grep -q 'tailwindcss' "$_css" 2>/dev/null; then
        dim "  $(basename "$_css") already imports tailwindcss"
    else
...
recipe_run() {
    ...
    # 5b ─ SDD plane (kit + flow skills) — opt-in via --sdd / wizard ────────────
